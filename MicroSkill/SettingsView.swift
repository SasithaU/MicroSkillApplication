            ...
            List {
                Section("Account") {
                    ...
                    Picker("Learning Focus", selection: $userGoal) {
                        ...
                        if !goalOptions.contains(userGoal) && !userGoal.isEmpty {
                            Text(userGoal).tag(userGoal)
                        }
                    }
                }
                ...
                Section("Gemini AI") {
                    NavigationLink(destination: APISettingsView()) {
                        HStack {
                            ...
                            Spacer()
                            Text(store.geminiApiKey.isEmpty ? "Not Configured" : "Configured")
                                .font(.caption)
                                .foregroundColor(store.geminiApiKey.isEmpty ? .orange : .green)
                        }
                    }
                }
                .listRowBackground(Color.white.opacity(0.05))
...
struct APISettingsView: View {
    ...
    private func saveKey() {
        ...
        store.geminiApiKey = trimmedKey
        apiKey = trimmedKey // Update UI
        showSavedAlert = true
    }
}
